...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const repository = body.repository?.full_name || `${body.repository?.owner?.login}/${body.repository?.name}` || "unknown"
		...
		if (event === "pull_request") {
			...
			if (action === "opened" || action === "synchronize") {
				...
				reviewPullRequest(owner, repo, prNumber)
					.then(() => {
						console.log(`[WEBHOOK] Review process completed for ${owner}/${repo}#${prNumber} (took ${Date.now() - startTime}ms)`);
					})
					.catch((error) => {
						console.error(`[WEBHOOK] Review process failed for ${owner}/${repo}#${prNumber}:`, error);
					});
